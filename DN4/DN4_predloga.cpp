...
int main() {
  ...
  for (int ii=0; ii<2000; ii++) //2000 iteracij
    {
      for (int i = 0; i < n; i++) {
          double sum = 0;
          for (int j = 0; j < n; j++) {
  ...
  cout << "Max. temperature: " << max_T << " degree C." << endl;//dobil sem najve?jo temperaturo 0.
  
    
  
  return 0;
}
//Paralelizacija ni možna. To je zato ker Gauss-Seidel metoda za vsako iteracijo ra?unanja temperature, 
// potrebuje že posodobljene vrednosti prejšnih elementov. Torej bi bilo potrebno nekako pridobiti nove 
// podatke za naslednjo temperaturo medtem ko ra?unamo trenutno.